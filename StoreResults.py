...
#                                   for (px, py) in self.player_positions]
#
#         self.player_points = [0.0 for a in self.prev_dist_to_food]
...
#         #agent_inits = [GreedyPredatorAgent(1), GreedyPredatorAgent(2)]
#         return agent_inits
#
#
...
# parser.add_argument('--num_episodes', type=int, default=250, help="Number of episodes for training")
...
#
# def create_parallel_env(args, player, num_envs, seed):
...
#                            max_time_steps=args['episode_length'], scheduler=scheduler, seed=seed) for scheduler
#                     in schedulers]
#     return envs
#
# def get_ip():
...
#         player.load_parameters("params_adhoc_rnn_first/params_"+str(20))
#         #player = TeammateAwarePredator(agent_id=0)
#         param_results = []
#
#         for eps_index in range(num_episodes):
#             envs = create_parallel_env(vars(args), player, arguments['num_envs'], seed=eps_index * 200)
...
#                 data = {"agent_pos": envs[0].player_positions,
#                         "agent_orientation": envs[0].player_orientation,
#                         "food_positions": envs[0].food_positions,
#                         "food_orientations": envs[0].food_orientation,
#                         "agent_types": [type(k) for k in envs[0].agent_list],
#                         "h_e": (player_acts[1][0][0].detach().numpy(), player_acts[1][0][1].detach().numpy()),
#                         "h_n": (player_acts[2][0][0].detach().numpy(), player_acts[2][0][1].detach().numpy()),
#                         "h_u": (player_acts[3][0].detach().numpy(), player_acts[3][1].detach().numpy()),
#                         "feat_e": player_acts[4].detach().numpy(),
#                         "feat_n": player_acts[5].detach().numpy(),
#                         "feat_u": player_acts[6].detach().numpy()}
#                 exp_data.append(data)
#                 player_obs = [env.step(player_act) for env, player_act in zip(envs, player_acts[0])]
#
...
#             print("Eps Done!!! Policy ",(exp_id+1) ," took these seconds : ", end-start, " with total rewards : ", total_rewards)
#             param_results.append(total_rewards)
#         results.append(param_results)
#     #with open('heur_team_in_full_adhoc_packsize.pkl', 'wb') as f:
#     #    pkl.dump(results, f)
#     with open('dataset_representation.pkl', 'wb') as f:
#         pkl.dump(dataset, f)
#
#
...
parser.add_argument('--num_predators', type=int, default=2, help='number of predators')
parser.add_argument('--num_food', type=int, default=2, help='number of preys')
parser.add_argument('--num_episodes', type=int, default=30, help="Number of episodes for training")
parser.add_argument('--update_frequency', type=int, default=10, help="Timesteps between updates")
parser.add_argument('--episode_length', type=int, default=200, help="Number of timesteps in episode")
parser.add_argument('--sampling_wait_time', type=int, default=100, help="timesteps until begin updating")
parser.add_argument('--saving_frequency', type=int,default=40,help="saving frequency")
...
if __name__ == '__main__':
    ...
    for exp_id in range(20):
        ...
        player = AdHocShortBPTTAgent(args=arguments, agent_id=0, with_added_u=True, added_u_dim=4)
        player.load_parameters("params_greedy/params_"+str(exp_id+1))
        #player = TeammateAwarePredator(agent_id=0)
        param_results = []
        ...
        for eps_index in range(num_episodes):
            ...
            print("Eps Done!!! Policy ",(exp_id+1) ," took these seconds : ", end-start, " with total rewards : ", total_rewards)
            if len(pack_size) > 0:
                param_results.append(sum(pack_size)/len(pack_size))
            else:
                param_results.append(0.0)
        results.append(param_results)
    with open('greedy_in_adhoc_average_pack_size.pkl', 'wb') as f:
        pkl.dump(results, f)
